...
std::string PokerGame::get_winning_hand_description() const {
    ...
    if (hand_evaluation.has_value()) {
        switch (hand_evaluation.value().category) {
            ...
            case PokerHandEvaluationCategory::StraightFlush:
                ...
                if (!hand_evaluation.value().tiebreakers.empty() && hand_evaluation.value().tiebreakers[0] == 14) { // Ace high
